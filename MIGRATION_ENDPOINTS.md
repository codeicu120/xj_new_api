...
| 接口 | PHP handler | 备注 |
| --- | --- | --- |
| `/payment/:action`（除 `/payment/index`、`/payment/query`、`/payment/unpaid`、`/payment/success`、`/payment/failed`） | `c.api.payment->$action` | 未重构；剩余 `payways/chpayway/reqpay` 等涉及支付方式、下单或平台请求；裸 `/payment` 旧 PHP 为 404 |
